...
import { ColorColumnSchema } from "@/lib/validation/color-column"
...
interface ColorTableRowActionsProps<TData> {
  row: Row<TData>
}

export function ColorTableRowActions<TData>({ row }: ColorTableRowActionsProps<TData>) {
  const router = useRouter()
  const { shopSlug } = useParams()
  const [isLoading, setIsLoading] = useState<boolean>(false)
  const [open, setOpen] = useState<boolean>(false)

  const color = ColorColumnSchema.parse(row.original)
  ...
  const onDelete = async () => {
    try {
      ...
      await axios.delete(`/api/${shopSlug}/colors/${color.id}`)
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          <DropdownMenuItem onClick={() => router.push(`/${shopSlug}/colors/${color.id}`)}>
